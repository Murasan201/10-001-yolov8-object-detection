...
- **ガイドライン従拠**: 以下の外部ガイドラインに従うこと
  - `COMMENT_STYLE_GUIDE.md`: コメント記載の標準化
  - `python_coding_guidelines.md`: Pythonコーディング規約（PEP 8、if __name__ == "__main__"、docstring、エラーハンドリング）
...
### 13. スタイルガイド準拠
本プロジェクトでは、以下の2つのスタイルガイドに従う必要があります。これらはプロジェクトルートに配置されています。

#### 13.1 COMMENT_STYLE_GUIDE.md
コメント記載の標準化ガイドです。以下の項目を遵守してください：

**ファイルヘッダー**
- シバン（`#!/usr/bin/env python3`）を含める
- プロジェクト名、簡潔な説明、要件定義書への参照を記載

**docstring**
- クラス：1-2行で役割を説明
- メソッド/関数：処理内容、Args、戻り値を明記
- 初心者向けの補足情報を含める

**インラインコメント**
- 複雑なロジック、初心者向け情報、トラブルシューティングの説明
- 自動変換ツール（flake8など）ではなく、人間が読みやすいコメントを心がける

**エラーハンドリング**
- エラーメッセージに処理名を含める
- ユーザー向けの対処方法と具体的コマンド例を提示

#### 13.2 python_coding_guidelines.md
Pythonコーディング規約のガイドです。以下の項目を遵守してください：

**必須項目**
- PEP 8に基本的に従う（わかりやすさを最優先）
- すべての実行可能ファイルに `if __name__ == "__main__":` を含める
- グローバル実行コードはすべて関数に移動
